...
def test_dev_db_insert_and_query_flow():
    ...
    insert_res = client.post("/api/v1/dev/db/tables/users/rows", json=insert_payload)
    ...
    get_res = client.get("/api/v1/dev/db/tables/users/rows?search=tester.dev@placify.internal")
    ...
    sql_res = client.post("/api/v1/dev/db/query", json=query_payload)
