    ...
    <div className={styles.container}>
      ...
      <div className={styles.quickStatsGrid}>
        <div className={styles.statBox}>
          <div className="text-xs text-gray-500 font-medium uppercase">Pending Uploads</div>
          <div className="text-2xl font-bold text-amber-600 mt-1">1 Request</div>
          <div className="text-xs text-gray-500 mt-0.5">From Placement Rep</div>
        </div>
        <div className={styles.statBox}>
          <div className="text-xs text-gray-500 font-medium uppercase">Letters Verified</div>
          <div className="text-2xl font-bold text-emerald-600 mt-1">1 Letter</div>
          <div className="text-xs text-gray-500 mt-0.5">By Faculty In-charge</div>
        </div>
        <div className={styles.statBox}>
          <div className="text-xs text-gray-500 font-medium uppercase">Top Campus Package</div>
          <div className="text-2xl font-bold text-indigo-600 mt-1">₹38.5 LPA</div>
          <div className="text-xs text-gray-500 mt-0.5">Uber India</div>
        </div>
        <div className={styles.statBox}>
          <div className="text-xs text-gray-500 font-medium uppercase">Placed Alumni</div>
          <div className="text-2xl font-bold text-gray-900 mt-1">3,400+</div>
          <div className="text-xs text-gray-500 mt-0.5">Available for Referrals</div>
        </div>
      </div>
      ...
      {uploadToast && (
        <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-4 flex items-start gap-3 text-emerald-900 text-sm shadow-sm transition-all animate-in fade-in">
          ...
          <div>
            ...
            <div className="text-xs text-emerald-800 mt-0.5">{uploadToast}</div>
          </div>
        </div>
      )}
      ...
      <div className="space-y-4">
        <div className={styles.sectionHeader}>
          ...
          <Badge variant="outline" className="text-gray-600 bg-gray-50 self-start sm:self-auto">
              ...
              <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                <div className="space-y-2">
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className="font-bold text-gray-900 text-base">{req.companyName}</span>
                  ...
                  <div className="text-xs text-gray-600 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-2 pt-1">
                    <div className="flex items-center gap-1.5">
                      <Briefcase className="w-3.5 h-3.5 text-gray-400" />
                      <span>Role: <strong className="text-gray-800">{req.role}</strong></span>
                    </div>
                    <div className="flex items-center gap-1.5">
                      <User className="w-3.5 h-3.5 text-gray-400" />
                      <span>Assigned by: <strong className="text-gray-800">{req.requestedBy}</strong></span>
                    </div>
                    <div className="flex items-center gap-1.5">
                      <ShieldCheck className="w-3.5 h-3.5 text-emerald-600" />
                      <span>Faculty In-charge: <strong className="text-gray-800">{req.inchargeFacultyName}</strong></span>
                    </div>
                    <div className="flex items-center gap-1.5">
                      <Calendar className="w-3.5 h-3.5 text-gray-400" />
                      <span>Deadline: <strong className="text-rose-600">{req.deadline}</strong></span>
                    </div>
                  </div>

                  {req.uploadedFileName && (
                    <div className="text-xs text-blue-700 bg-blue-50/60 border border-blue-100 rounded-md px-2.5 py-1.5 inline-flex items-center gap-1.5 mt-1 font-mono">
                      <FileCheck2 className="w-3.5 h-3.5 text-blue-600" /> Stored in College Shared Drive: {req.uploadedFileName}
                    </div>
                  )}
                </div>
                ...
                <div className="shrink-0 flex items-center gap-2">
                  {req.status === "pending_upload" ? (
                    <Button
                      ...
                      className="gap-2 bg-blue-600 hover:bg-blue-700 text-xs h-9 shadow-sm"
                    >
                      <UploadCloud className="w-4 h-4" /> Upload Offer Letter
                    </Button>
                  ) : req.status === "processing" ? (
                    <Button variant="outline" size="sm" disabled className="text-xs text-blue-600 bg-blue-50/30 border-blue-200">
                      Processing with AI
                    </Button>
                  ) : (
                    <Button variant="outline" size="sm" disabled className="text-xs text-emerald-700 border-emerald-200 bg-emerald-50/30">
            ...
            <table className="w-full text-left text-sm text-gray-700">
              <thead className={styles.tableHeader}>
                <tr>
                  ...
                  <th className="px-6 py-3.5 text-right">Referral Matching (Agent 6)</th>
                </tr>
              </thead>
                ...
                {filteredPlacements.map((p) => (
                  <tr key={p.id} className="hover:bg-blue-50/30 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      ...
                      <div className="text-gray-500 text-[11px] mt-0.5">{p.role}</div>
                    </td>
                    <td className="px-6 py-4">
                      <span className="font-bold text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded border border-emerald-100">
                        ₹{p.packageLPA} LPA
                      </span>
                      <div className="text-[10px] text-gray-400 mt-1 uppercase font-semibold">{p.offerType}</div>
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4 text-right">
                      {p.isSenior ? (
                        <Button
                          size="sm"
                          variant="outline"
                          className="h-7 text-xs gap-1 border-indigo-200 text-indigo-700 hover:bg-indigo-50 font-medium cursor-pointer"
                        >
                          <Sparkles className="w-3 h-3 text-amber-500" /> Request Referral
                        </Button>
                      ) : (
                        <span className="text-gray-400 text-xs italic">Campus Peer</span>
                      )}
                    </td>
                  </tr>
                ))}
        ...
        <div className={styles.modalOverlay}>
          <div className={styles.modalContent}>
            <div className="flex items-center justify-between border-b pb-3">
              <div>
                <h3 className="font-bold text-gray-900 text-base">Submit Offer Letter Document</h3>
                <p className="text-xs text-gray-500 mt-0.5">For {activeUploadRequest.companyName} ({activeUploadRequest.offerType})</p>
              </div>
              <button
            ...
            <div className="space-y-4 text-xs text-gray-600">
              <div className="bg-slate-50 border border-slate-200 rounded-lg p-3 space-y-1">
                <div className="text-slate-700 font-medium">Assigned In-charge Faculty:</div>
              ...
              {/* Upload Dropzone */}
              <div className="border-2 border-dashed border-blue-200 hover:border-blue-400 bg-blue-50/40 rounded-xl p-6 text-center space-y-2 cursor-pointer transition-colors">
                <div className="w-10 h-10 rounded-full bg-blue-100 text-blue-600 mx-auto flex items-center justify-center">
                  <UploadCloud className="w-5 h-5" />
                </div>
                <div className="text-xs text-gray-700 font-medium">
                  Selected File: <span className="font-mono text-blue-700 font-semibold">{stagedFileName}</span>
                </div>
                <p className="text-[11px] text-gray-400">PDF, PNG, JPG accepted (Max 10MB)</p>
              </div>
